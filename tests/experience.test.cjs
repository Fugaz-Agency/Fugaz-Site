...
function environment({fontsPending=false, imagesFail=false, hidden=false}={}) {
  ...
  const pre={removeAttribute:()=>{}};
  const root={querySelector:()=>pre,querySelectorAll:()=>[{decode:()=>imagesFail?Promise.reject(new Error('image failed')):Promise.resolve()}]};
...
(async()=>{
 ...
 for(const options of [{},{imagesFail:true}]){
  const t=environment(options);t.api.prepareIntro(t.root,t.start,t.skip);await flush();assert.equal(t.started,0);t.paint();assert.equal(t.started,0);t.paint();assert.equal(t.started,1);assert.equal(t.timers.size,0);
 }
 ...
 console.log('PASS: 12 planning months, Amsterdam date boundaries, year rollover, manual capacity (including zero), two-frame readiness, asset failures, bounded wait, cancellation, Escape, background tabs, and input modality.');
