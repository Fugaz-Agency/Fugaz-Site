...
(() => {
  ...
  function parseSiteDocument(doc) {
    ...
    return {
      ...
      js: scriptEl?.getAttribute("data-controller") === "fugaz-page" ? "fugaz-page" : scriptEl ? scriptEl.textContent || "" : "",
      props,
      preview
    };
  }
  ...
  function evalSiteLogic(src) {
    if (src === "fugaz-page") {
      if (typeof window.createFugazPage !== "function") throw new Error("Fugaz page controller did not load");
      return window.createFugazPage(StreamableLogic, getReact());
    }
