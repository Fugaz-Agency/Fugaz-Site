...
window.createFugazPage = (SiteLogic, React) => class FugazPage extends window.withFugazNavigation(window.withFugazScan(SiteLogic)) {
  ...
  applyAvailability() {
    const { spots, quarter, label } = window.FugazExperience.availability(new Date(), this.props.spots, this.props.quarter);
    document.querySelectorAll("[data-spot-text]").forEach(el => { el.textContent = label; });
    document.querySelectorAll("[data-spot-label]").forEach(el => { el.textContent = "spots left for " + quarter; });
    document.querySelectorAll("[data-spot-dot]").forEach(el => {
      el.style.background = spots > 0 ? "#2CCD6B" : "#CD2C2C";
    });
    document.querySelectorAll("[data-spot-count]").forEach(el => {
      el.textContent = String(spots);
      el.setAttribute("data-count", String(spots));
    });
  }
  componentDidMount() {
    try {
  ...
  bootInner() {
    ...
    if (preEl) {
      preEl.style.display = this.playIntro ? "block" : "none";
      if (this.playIntro) {
        preEl.setAttribute("data-preparing", "1");
        document.documentElement.style.overflow = "hidden";
        this.locked = true;
      }
    }
    this.cleanup.push(window.FugazExperience.trackInputMode());
    const availabilityTimer = setInterval(() => this.applyAvailability(), 60 * 60 * 1000);
    this.cleanup.push(() => clearInterval(availabilityTimer));
    this.on(document, "visibilitychange", () => { if (!document.hidden) this.applyAvailability(); });
    const bail = setTimeout(() => {
      if (this.introDone || document.hidden) return;
      this.introDone = true;
    ...
    const frame = () => {
      ...
      if (bar) bar.style.setProperty("--scroll-progress", (maxY() ? y / maxY() * 100 : 0).toFixed(2) + "%");
      if (nav) {
      ...
      if (pinWrap && pinInner && moms.length) {
        ...
        // Keep the mobile progress line visible while the dark panel fills the screen.
        if (bar) bar.classList.toggle("is-on-dark", vw <= 700 && this.pinHeld && kze > 0.98);
    ...
    };
    {
      ...
      const warm = () => {
        if (thrifty || (this.playIntro && !this.introDone)) return;
    ...
    if (this.playIntro && !this.introDone) {
      ...
      const cancelPreparation = window.FugazExperience.prepareIntro(root, () => {
        if (this.bootId === bootId && !this.introDone) this.runIntro();
      }, () => {
        this.introDone = true;
      ...
      this.cleanup.push(cancelPreparation);
    }
    if (!this.playIntro || this.introDone) {
      void root.offsetHeight;
      enterHero();
