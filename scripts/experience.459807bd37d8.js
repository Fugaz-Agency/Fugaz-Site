...
window.FugazExperience = {
  ...
  prepareIntro(root, start, skip) {
    ...
    let secondFrame = 0;
    let timeout;
    // The intro is a sibling of the main page, not a child of root.
    const preloader = root.querySelector("[data-preloader]") || document.querySelector("[data-preloader]");
    const cleanup = () => {
      cancelled = true;
    ...
    const images = Array.from(new Set([
      ...root.querySelectorAll("#top img, [data-mnav] img"),
      ...(preloader ? preloader.querySelectorAll("img") : [])
    ]));
